...
public class SkaiaQuad : Quad {
    ...
    public SkaiaQuad(Quad parent, Spatial LODTarget, int size, int depth, Vector3[] corners, Color color, float[] LODLevels, OpenSimplexNoise noiseGenerator) : 
    ...
    public override Vector3[] ConstructMesh()
    {
        ...
        for (int y = 0; y < _size; y++)
        {
            for (int x = 0; x < _size; x++)
            {
                ...
                vertices[i] = pointOnUnitSphere * (1 + (noise * .25f));
    ...
    public override void Subdivide()
    {
        for (int i = 0; i < CHILD_COUNT; i ++) {
            var child = new SkaiaQuad(this, _lodtarget, _size, _depth + 1, childCorners[i], ChildColors[i], _LODLevels, noiseGenerator);
            child.Name = "Child_" + i;
